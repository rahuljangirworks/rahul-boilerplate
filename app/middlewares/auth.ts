...
const GUEST_ONLY_ROUTES = ["/auth/sign-in", "/auth/sign-up", "/auth/forgot-password", "/auth/reset-password"] as const;
...
export const optionalAuthContext = createContext<BetterAuthSession | null>(null);

export const requiredAuthContext = createContext<BetterAuthSession>();
...
export const getSession = cache(async (request: Request) => {
  try {
    ...
    console.error("Error retrieving Better Auth session in middleware:", error);
    return null;
  }
});
...
export const authMiddleware: MiddlewareFunction = async (
  ...
  if (isProtectedRoute(pathname)) {
    ...
    context.set(optionalAuthContext, validatedData);
    context.set(requiredAuthContext, validatedData);
    return next();
  }
